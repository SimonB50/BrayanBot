  ...
  async initialize() {
    ...
    for (let i = 0; i < this.configFiles.length; i++) {
      const { default: config } = await import(
        `file://` + path.join(this.configDir, this.configFiles[i])
      );
...
export class Config {
  ...
  createFile() {
    if (!fs.existsSync("data/")) fs.mkdirSync("data/");
    fs.writeFileSync(this.storagePath, this.formatFile(), {
      encoding: "utf-8",
      flag: "w",
    });
    return this;
      ...
      .replace(
        ...
        (match) => {
          ...
          return (
            ...
            comment
              .substring(
                comment.startsWith('"') || comment.startsWith("'") ? 1 : 0,
